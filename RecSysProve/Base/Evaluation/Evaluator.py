...
class Evaluator(object):
    ...
    def __init__(self, URM_test_list, cutoff_list, minRatingsPerUser=1, exclude_seen=True,
                        ...
                        ignore_items = None,
                        ignore_users = None, random_users=None):
        ...
        if random_users is not None:
            self.usersToEvaluate = np.random.permutation(self.usersToEvaluate)
            self.usersToEvaluate = self.usersToEvaluate[:random_users]
    ...
    def _run_evaluation_on_selected_users(self, recommender_object, usersToEvaluate):
        ...
        for test_user in tqdm(usersToEvaluate):
            ...
            recommended_items = recommender_object.recommend(test_user, exclude_seen=self.exclude_seen,
                                                             at=self.max_cutoff)
...
class SequentialEvaluator(Evaluator):
    ...
    def __init__(self, URM_test_list, cutoff_list, minRatingsPerUser=1, exclude_seen=True,
        ...
        super(SequentialEvaluator, self).__init__(URM_test_list, cutoff_list,
                            ...
                            ignore_items = ignore_items, ignore_users = ignore_users, random_users=random_users)
    ...
    def _run_evaluation_on_selected_users(self, recommender_object, usersToEvaluate, block_size = 1000):
        ...
        prog_bar = tqdm(total=len(usersToEvaluate))
        while user_batch_start < len(usersToEvaluate):
            ...
            for batch_user_index in range(len(recommended_items_batch_list)):
                ...
                # if time.time() - start_time_print > 15 or n_users_evaluated==len(self.usersToEvaluate):
                #     print("SequentialEvaluator: Processed {} ( {:.2f}% ) in {:.2f} seconds. Users per second: {:.0f}".format(
                #                   n_users_evaluated,
                #                   100.0* float(n_users_evaluated)/len(self.usersToEvaluate),
                #                   time.time()-start_time,
            ...
            prog_bar.update(len(test_user_batch_array))
        prog_bar.close()
        return results_dict, n_users_evaluated
