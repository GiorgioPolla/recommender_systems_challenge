...
class HybridWithSlim(object):
    def __init__(self, urm, icm, w1=4, w2=3, comb='linear'):
        self.urm = urm
    ...
    def recommend(self, user_id, at=10, exclude_seen=True, only_scores=False):
        scores_hyb = self.hybrid_rec.recommend(user_id, at=at, exclude_seen=exclude_seen, only_scores=True).toarray().ravel()
        scores_slim = self.slim_rec.recommend(user_id, at=at, exclude_seen=exclude_seen, only_scores=True)
        scores_slim = minmax_scale(scores_slim)
        ranking = None
