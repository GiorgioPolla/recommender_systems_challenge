...
from misc.othe import filter_seen
...
class MF_MSE_PyTorch(Recommender, Incremental_Training_Early_Stopping):
    ...
    def recommend(self, user_id, at=10, exclude_seen=True, only_scores=False):
        ...
        if exclude_seen:
            scores = filter_seen(self.URM_train, user_id, scores)

        if only_scores:
            return scores

        return scores.argsort()[::-1][:at]




    def fit(self, epochs=10, batch_size = 128, num_factors=10,
            learning_rate = 0.001,
        ...
        from recommenders.matrix_factorization.pytorch.MF_MSE_PyTorch_model import MF_MSE_PyTorch_model, DatasetIterator_URM
